...
function system.build(stub, diplomacy, day)
    ...
    for i = 1, planetCount do
        ...
        local body = {
            ...
            temperature = temp,
            orbitRadius = orbit,
            orbitPeriod = 90 * math.sqrt((orbit / S.orbitMin) ^ 3) * prng:range(0.85, 1.2),
            orbitPhase = prng:range(0, TAU),
            orbitInclination = prng:gauss(0, 0.05),
            dayLength = prng:range(0.4, 3.2),
        ...
        for m = 1, moonCount do
            ...
            body.moons[m] = {
                ...
                temperature = temp,
                orbitRadius = morbit,
                orbitPeriod = 2.5 * math.sqrt((morbit / (body.radius * 6)) ^ 3) * mrng:range(0.8, 1.3),
                orbitPhase = mrng:range(0, TAU),
                orbitInclination = mrng:gauss(0, 0.12),
                dayLength = mrng:range(1, 8),
